...
pub(crate) const TRANSLATIONS_TABLE_SQL: &str = r#"CREATE TABLE translations (
...
pub(crate) const TRANSLATIONS_INDEX_SQL: &str =
    "CREATE INDEX translations_last_accessed_idx ON translations(last_accessed_at)";
...
pub(crate) fn migrate(connection: &mut Connection) -> rusqlite::Result<()> {
    ...
    match version {
        0 => {
            ...
            transaction.execute_batch(TRANSLATIONS_TABLE_SQL)?;
            transaction.execute_batch(TRANSLATIONS_INDEX_SQL)?;
